...
def gradient_descend(
    tr_images, tr_labels, alpha, iterations, layers=1, neurons=10, final=10
):
    """
    梯度下降
    ...
    for i in range(iterations):
        Zs_h, As_h, Z_o, A_o = forward_prop(
            Ws_h, Bs_h, W_o, B_o, tr_images, ReLU
        )  # 前向传播
        dWs_h, dBs_h, dW_o, dB_o = back_prop(
            Zs_h, As_h, Z_o, A_o, Ws_h, W_o, tr_images, tr_labels, ReLU_deriv
        )  # 反向传播
        Ws_h, Bs_h, W_o, B_o = update_params(
            Ws_h, Bs_h, W_o, B_o, dWs_h, dBs_h, dW_o, dB_o, alpha
        )  # 更新参数
        if i % 10 == 0:
            print("Iteration: ", i)
